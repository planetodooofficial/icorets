...
class LocationReport(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, lines):
        ...
        sheet.write(0, 23, 'Colour', bold)
        sheet.write(0, 24, 'Size', bold)
        sheet.write(0, 25, 'MRP', bold)
        sheet.write(0, 26, 'GST', bold)
        sheet.write(0, 27, 'IHO Stock', bold)
        sheet.write(0, 28, 'Bhiwandi Stock', bold)
        sheet.write(0, 29, 'Delhi Stock', bold)
        sheet.write(0, 30, 'Total Qty', bold)
        sheet.write(0, 31, 'Quotation Qty', bold)
        sheet.write(0, 32, 'Confirmed SO qty', bold)
        sheet.write(0, 33, 'FREE TO USE', bold)
        sheet.write(0, 34, 'PO Receipt Pending', bold)
        sheet.write(0, 35, 'To Replenish', bold)
        sheet.write(0, 36, 'Product Net Weight (gms)', bold)
        sheet.write(0, 37, 'Product Dimension (cm)', bold)
        sheet.write(0, 38, 'Product Dimension (cm)', bold)
        sheet.write(0, 39, 'Product Dimension (cm)', bold)
        sheet.write(0, 40, 'Package Weight (gms)', bold)
        sheet.write(0, 41, 'Package Dimension (cm)', bold)
        sheet.write(0, 42, 'Package Dimension (cm)', bold)
        ...
        for product_id, data in sorted(product_data.items(), key=lambda x: (x[1]['Brand'].lower(), x[1]['Category 1'].lower(), x[1]['Category 2'].lower(), x[1]['Category 3'].lower())):
            ...
            sheet.write(row, col + 23, data['Colour'])
            sheet.write(row, col + 24, data['Size'])
            sheet.write(row, col + 25, data['MRP'])
            sheet.write(row, col + 26, data['GST'])
            sheet.write(row, col + 27, data['IHO Stock'])
            sheet.write(row, col + 28, data['Bhiwandi Stock'])
            sheet.write(row, col + 29, data['Delhi Stock'])
            sheet.write(row, col + 30, data['Total'])
            sheet.write(row, col + 31, data['Quotation Qty'])
            sheet.write(row, col + 32, data['Confirmed SO qty'])
            sheet.write(row, col + 33, data['FREE TO USE'])
            sheet.write(row, col + 34, data['PO Receipt Pending'])
            sheet.write(row, col + 35, data['To Replenish'])
