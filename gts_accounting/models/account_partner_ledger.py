...
class PartnerLedgerCustomHandler(models.AbstractModel):
        ...
        for column in options['columns']:
            ...
            if col_value is None:
                ...
                if col_expr_label == 'date_maturity':
                    ...
                    credit_vat = sum(
                        move_line.move_id.line_ids.filtered(lambda x: x.account_id.is_vat).mapped('credit'))
                    if debit_vat > 0:
                        amount = debit_vat
                    else:
                        amount = credit_vat
    ...
    def _query_partners(self, options):
        ...
        if groupby_partners:
            ...
            partners = self.env['res.partner'].with_context(active_test=False, prefetch_fields=False).search(
    ...
    def _get_query_sums(self, options):
        ...
        for column_group_key, column_group_options in report._split_options_per_column_group(options).items():
            ...
            queries.append(f"""
                WITH vat_tds_sums AS (
                    SELECT
                        aml_vat_tds.move_id AS move_id,
                        SUM(
                            ROUND(
                                CASE
                    ...
                    FROM account_move_line aml_vat_tds
                    JOIN account_account acc ON aml_vat_tds.account_id = acc.id
                    JOIN {ct_query} ON currency_table.company_id = aml_vat_tds.company_id
                    GROUP BY aml_vat_tds.move_id
                )
                SELECT
                    ...
                    COALESCE(vat_tds_sums.vat, 0) AS vat,
                    COALESCE(vat_tds_sums.tds, 0) AS tds
                FROM {tables}
                LEFT JOIN {ct_query} ON currency_table.company_id = account_move_line.company_id
                LEFT JOIN account_move am ON account_move_line.move_id = am.id
                LEFT JOIN vat_tds_sums ON am.id = vat_tds_sums.move_id
                WHERE {where_clause}
                GROUP BY account_move_line.partner_id, vat_tds_sums.vat, vat_tds_sums.tds
            """)
    ...
    def _get_sums_without_partner(self, options):
        ...
        for column_group_key, column_group_options in report._split_options_per_column_group(options).items():
            ...
            queries.append(f"""
                ...
                GROUP BY aml_with_partner.partner_id, vat_tds_sums.vat, vat_tds_sums.tds
            """)
